...
func (j *Joystick) RenderAndListen(ctx *scene.Context, joy *joystick.Joystick, layer int) error {
	...
	b1 := event.Bind(ctx.Handler, joystick.Disconnected, j, func(rend *Joystick, _ uint32) event.Response {
		j.Destroy()
		return 0
	})

	// TODO: it is bad that you need to import two 'key' packages
	b2 := event.Bind(ctx.Handler, key.Down(mkey.CodeSpacebar), j, func(j *Joystick, _ key.Event) event.Response {
	...
	b3 := event.Bind(ctx.Handler, joystick.Change, j, func(j *Joystick, st *joystick.State) event.Response {
	...
	b4 := event.Bind(ctx.Handler, joystick.LtStickChange, j, func(j *Joystick, st *joystick.State) event.Response {
		pos := j.lStickCenter
	...
	b5 := event.Bind(ctx.Handler, joystick.RtStickChange, j, func(j *Joystick, st *joystick.State) event.Response {
		pos := j.rStickCenter
		pos = pos.Add(floatgeom.Point2{
